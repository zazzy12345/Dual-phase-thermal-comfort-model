...
def Blur(img, kernel):
    blur = cv2.GaussianBlur(img, (kernel, kernel), 1.5)
...
# 测试一组图片
def TestOneDir():
    root_path = "D:/Github/deep-learning-for-image-processing-master/deep-learning-for-image-processing-master/data_set/final_thermalFace_split_balance/train/warm"
    save_path = root_path
    for a, b, c in os.walk(root_path):
        for file_i in c:
            ...
            #
            img_rotate = Rotate(img_i, 10)
            cv2.imwrite(os.path.join(save_path, file_i[:-4] + "_rotate10.jpg"), img_rotate)
            img_rotate = Rotate(img_i, -10)
            cv2.imwrite(os.path.join(save_path, file_i[:-4] + "_rotate-10.jpg"), img_rotate)
            #
